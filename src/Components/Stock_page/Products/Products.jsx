      ...
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            ...
            // Remove from AsyncStorage (only saved products, not defaults)
            const prodData = await AsyncStorage.getItem('products');
            const savedProds = prodData ? JSON.parse(prodData) : [];
            const updatedSaved = savedProds.filter((p) => p.id !== product.id);
            await AsyncStorage.setItem('products', JSON.stringify(updatedSaved));
